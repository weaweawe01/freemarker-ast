...
type unsafeMethodIndex struct {
	byClass map[string]map[string]struct{}
}

func (idx unsafeMethodIndex) has(className, methodName string) bool {
	methods, ok := idx.byClass[className]
	if !ok {
		return false
	}
	_, ok = methods[methodName]
	return ok
}

var legacyUnsafeMethods = parseUnsafeMethodIndex(unsafeMethodsRaw)

func parseUnsafeMethodIndex(raw string) unsafeMethodIndex {
	idx := unsafeMethodIndex{
	...
	for scanner.Scan() {
		...
		lastDot := strings.LastIndex(line[:openParen], ".")
		if lastDot <= 0 {
			continue
		}

		className := normalizeClassName(line[:lastDot])
		methodName := strings.ToLower(strings.TrimSpace(line[lastDot+1 : openParen]))
		if className == "" || methodName == "" {
			continue
		}

		methods := idx.byClass[className]
		if methods == nil {
			methods = make(map[string]struct{})
			idx.byClass[className] = methods
		}
